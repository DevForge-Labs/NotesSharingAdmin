...
interface PyqsSubjectGroupCardProps {
  group: { id: string; name: string; pyqs: PyqItem[] };
  isSelectionMode: boolean;
  selectedIds: string[];
  setSelectedIds: React.Dispatch<React.SetStateAction<string[]>>;
  allSelected: boolean;
  handleSelectAll: () => void;
  handleOpenDetails: (item: PyqItem) => void;
  formatFileSize: (bytes: any) => string;
}

const PyqsSubjectGroupCard: React.FC<PyqsSubjectGroupCardProps> = ({
  group,
  isSelectionMode,
  selectedIds,
  setSelectedIds,
  ...
  const { visibleItems, isExpanded, toggleExpand, hasMore, remainingCount } = useExpandableList(group.pyqs, 5);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Subject Group • {group.pyqs.length} {group.pyqs.length === 1 ? 'PYQ Paper' : 'PYQ Papers'}
            </p>
          </div>
        </div>
        <Badge variant="secondary" className="font-semibold text-xs bg-accent text-foreground">
          {group.pyqs.length} PYQs
        </Badge>
      </div>
              ...
              {visibleItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="p-4 text-xs font-mono">{formatFileSize(item.fileSize)}</td>
...
export const Pyqs: React.FC = () => {
  ...
  const fetchPyqs = async () => {
    ...
    try {
      ...
      querySnapshot.forEach(docSnap => {
        const data = docSnap.data() as PyqItem;
        if (!data.temp) {
          fetched.push({
            ...data,
  ...
  const handleOpenDetails = (pyq: PyqItem) => {
    setSelectedPyq(pyq);
  ...
  const filteredPyqs = useMemo(() => {
    return pyqs.filter(item => {
      ...
      if (q) {
        ...
        const examMatch = item.examType?.toLowerCase().includes(q) || item.examYear?.toLowerCase().includes(q) || false;
        const idMatch = item.id?.toLowerCase().includes(q) || false;
        if (!titleMatch && !subjectMatch && !uploaderMatch && !examMatch && !idMatch) return false;
      }
      return true;
    });
  }, [pyqs, selectedCollege, selectedBranch, selectedSemester, searchQuery]);
  ...
  const subjectGroups = useMemo(() => {
    ...
    const groupsMap = new Map<string, { id: string; name: string; pyqs: PyqItem[] }>();

    catalogSubjects.forEach(s => {
      groupsMap.set(s.id.toLowerCase(), { id: s.id, name: s.name, pyqs: [] });
    });

    const uncategorized: PyqItem[] = [];

    sortedPyqs.forEach(item => {
      ...
      if (matchedKey) {
        groupsMap.get(matchedKey)!.pyqs.push(item);
      } else {
        ...
        if (customSubName) {
          ...
          if (!groupsMap.has(customKey)) {
            groupsMap.set(customKey, { id: customKey, name: customSubName, pyqs: [] });
          }
          groupsMap.get(customKey)!.pyqs.push(item);
        } else {
    ...
    const result = Array.from(groupsMap.values()).filter(grp => grp.pyqs.length > 0);
    ...
    return result;
  }, [sortedPyqs, currentStage, getSubjects, selectedCollege, selectedBranch, selectedSemester]);

  const allSelected = sortedPyqs.length > 0 && sortedPyqs.every(item => selectedIds.includes(item.id));

  const handleSelectAll = () => {
    if (allSelected) {
      const sortedIds = sortedPyqs.map(n => n.id);
      setSelectedIds(prev => prev.filter(id => !sortedIds.includes(id)));
    } else {
      const newIds = new Set([...selectedIds, ...sortedPyqs.map(n => n.id)]);
      setSelectedIds(Array.from(newIds));
    }
  };

  const getNoteCountForCollege = (colId: string) => {
    if (colId === 'all') return pyqs.length;
    return pyqs.filter(n => (n.college || 'kiit').toLowerCase() === colId.toLowerCase()).length;
  };

  const getNoteCountForBranch = (branchId: string, colId?: string) => {
    const targetCol = colId || selectedCollege;
    if (branchId === 'all') {
      if (!targetCol || targetCol === 'all') return pyqs.length;
      return pyqs.filter(n => (n.college || 'kiit').toLowerCase() === targetCol.toLowerCase()).length;
    }
    return pyqs.filter(n => {
  ...
  const getNoteCountForSemester = (semId: string) => {
    if (semId === 'all') return filteredPyqs.length;
    return pyqs.filter(n => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative">
          ...
          <h2 className="text-2xl font-bold tracking-tight font-heading pl-3">Previous Year Questions (PYQs)</h2>
          <p className="text-sm text-muted-foreground pl-3">
            Audit, verify, and organize mid-semester and end-semester question paper archives.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Choose an institution from the dynamic App Catalog to view available PYQ papers.
              </p>
            </div>
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForCollege('all')} total PYQs across all institutions
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        View PYQ papers across all departments in all colleges
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <Badge variant="secondary" className="text-xs font-bold px-3 py-1 bg-background">
                      {getNoteCountForBranch('all', 'all')} PYQs
                    </Badge>
                          ...
                          <CardContent className="p-4 flex flex-col justify-between h-32">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-muted-foreground mt-0.5">
                                {colCount} PYQs in {collegeItem.name}
                              </p>
                            </div>
                          </CardContent>
                        </Card>
              ...
              >
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForBranch('all')} PYQs across all departments
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            >
              ...
              <CardContent className="p-5 flex flex-col justify-between h-36">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {getNoteCountForSemester('all')} total PYQs in branch
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      {/* STAGE 4: PYQS DISPLAY GROUPED BY SUBJECT */}
      {currentStage === 4 && (
        <div className="space-y-6">
          <Card className="border-border bg-card/50 backdrop-blur-sm shadow-premium">
            <CardContent className="p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  type="text"
                  placeholder="Search PYQs by title, subject, exam type, exam year, or document ID..."
                  value={searchQuery}
          ...
          {isSelectionMode && (
            <div className="flex items-center gap-2 px-4 py-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl text-xs font-semibold animate-fade-in">
              ...
              <span>Selection Mode Active — Select PYQs to delete.</span>
            </div>
          )}
            ...
            <Card className="border-border overflow-hidden shadow-premium">
              <CardContent className="p-16 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mt-1">
                  {searchQuery
                    ? 'No uploaded PYQ papers match your search parameters in this selection.'
                    : 'No PYQ documents are currently available for this selection.'}
                </p>
                {searchQuery ? (
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <DialogHeader className="border-b border-border/80 pb-4 mb-4 text-left shrink-0 pr-8">
              <div className="flex items-start gap-4">
                <div className="p-4 rounded-2xl border shrink-0 bg-amber-500/10 text-amber-500 border-amber-500/20">
                  <Layers className="h-8 w-8" />
                </div>
                <div className="min-w-0 flex-1">
                  <DialogTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2 flex-wrap" title={selectedPyq.title}>
                  ...
                  <div
                    className={`flex items-center gap-1 mt-2 text-xs font-bold px-2.5 py-0.5 rounded-full w-max ${
                      selectedPyq.isVerified
                        ? 'text-emerald-500 bg-emerald-500/10 border border-emerald-500/20'
                        : 'text-amber-500 bg-amber-500/10 border border-amber-500/20'
                    }`}
                  >
                    {selectedPyq.isVerified ? <CheckCircle className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                    {selectedPyq.isVerified ? 'Verified Document' : 'Pending Verification'}
                  </div>
                </div>
              </div>
            </DialogHeader>
                ...
                {selectedPyq.thumbnailUrl ? (
                  <div
                    className={`w-full max-w-xs h-32 bg-accent/20 rounded-xl overflow-hidden border border-border flex items-center justify-center shadow-sm relative group ${
                      selectedPyq.fileUrl || selectedPyq.downloadUrl ? 'cursor-pointer transition-all duration-200 hover:scale-[1.01] hover:opacity-90' : ''
                    }`}
                    onClick={() => {
                      const url = selectedPyq.fileUrl || selectedPyq.downloadUrl;
                      if (url) window.open(url, '_blank', 'noopener,noreferrer');
                    }}
                  >
                    <img src={selectedPyq.thumbnailUrl} alt="Preview thumbnail" className="w-full h-full object-cover group-hover:brightness-110 transition-all duration-200" />
                    {(selectedPyq.fileUrl || selectedPyq.downloadUrl) && (
                ...
                <Card className="border-border bg-accent/15 p-4 flex flex-col justify-between space-y-3">
                  ...
                  <div className="space-y-2 text-xs">
                    <div>
                      ...
                      <span className="font-semibold text-foreground">{selectedPyq.displaySubject || selectedPyq.subject || '—'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <span className="text-muted-foreground block text-[9px] uppercase tracking-wider font-semibold">Branch</span>
                        <span className="font-semibold text-foreground">{selectedPyq.branch || '—'}</span>
                      </div>
                      <div>
                        <span className="text-muted-foreground block text-[9px] uppercase tracking-wider font-semibold">Semester</span>
                        <span className="font-semibold text-foreground">{selectedPyq.semester || '—'}</span>
                      </div>
                    </div>
                    <div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-card border border-border/80 rounded-xl p-3 text-center shadow-sm">
                    ...
                    <span className="text-lg font-bold text-foreground mt-0.5 block">
                      {(selectedPyq.upvotes !== undefined ? selectedPyq.upvotes : selectedPyq.likesCount || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <Card className="border-border bg-accent/15 p-4 space-y-3">
                ...
                <div className="space-y-3 text-xs">
                  <div>
                    ...
                    <span className="font-semibold text-foreground">{selectedPyq.uploaderName || 'Anonymous'}</span>
                  </div>
                </div>
              </Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  <Button
                    ...
                    onClick={() => {
                      ...
                      else showToast('No File URL available', 'error');
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      const url = selectedPyq.fileUrl || selectedPyq.downloadUrl;
                      if (url) handleCopyText(url, 'fileUrl');
                      else showToast('No File URL available', 'error');
                    }}
                  >
                  ...
                  <Button
                    ...
                    size="sm"
                    className="text-xs font-bold flex items-center gap-1.5 h-10 bg-card border-border/80 text-foreground hover:bg-accent/20"
                    onClick={() => handleCopyText(selectedPyq.id, 'docId')}
                  >
      ...
      <BulkDeleteDialog
        ...
        showToast={showToast}
        resources={pyqs.filter(n => selectedIds.includes(n.id))}
        resourceType="pyqs"
      />
