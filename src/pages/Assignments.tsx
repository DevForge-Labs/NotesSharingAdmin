...
export const Assignments: React.FC = () => {
  ...
  const handleSelectAll = () => {
    if (allSelected) {
      const sortedIds = sortedAssignments.map(item => item.id);
      setSelectedIds(prev => prev.filter(id => !sortedIds.includes(id)));
    } else {
      const newIds = new Set([...selectedIds, ...sortedAssignments.map(item => item.id)]);
  ...
  return (
    <div className="space-y-6">
      ...
      <BulkDeleteDialog
        ...
        showToast={showToast}
        resources={assignments.filter(a => selectedIds.includes(a.id))}
        resourceType="assignments"
      />
