...
interface AssignmentsSubjectGroupCardProps {
  group: { id: string; name: string; assignments: AssignmentItem[] };
  isSelectionMode: boolean;
  selectedIds: string[];
  setSelectedIds: React.Dispatch<React.SetStateAction<string[]>>;
  allSelected: boolean;
  handleSelectAll: () => void;
  handleOpenDetails: (item: AssignmentItem) => void;
  formatFileSize: (bytes: any) => string;
}

const AssignmentsSubjectGroupCard: React.FC<AssignmentsSubjectGroupCardProps> = ({
  group,
  isSelectionMode,
  selectedIds,
  setSelectedIds,
  ...
  const { visibleItems, isExpanded, toggleExpand, hasMore, remainingCount } = useExpandableList(group.assignments, 5);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Subject Group • {group.assignments.length} {group.assignments.length === 1 ? 'Assignment' : 'Assignments'}
            </p>
          </div>
        </div>
...
export const Assignments: React.FC = () => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      querySnapshot.forEach(docSnap => {
        const data = docSnap.data() as AssignmentItem;
  ...
  const handleOpenDetails = (assignment: AssignmentItem) => {
  ...
  }, [assignments, selectedCollege, selectedBranch, selectedSemester, searchQuery]);
  ...
  const subjectGroups = useMemo(() => {
    ...
    const groupsMap = new Map<string, { id: string; name: string; assignments: AssignmentItem[] }>();

    catalogSubjects.forEach(s => {
      groupsMap.set(s.id.toLowerCase(), { id: s.id, name: s.name, assignments: [] });
    });

    const uncategorized: AssignmentItem[] = [];
    ...
    sortedAssignments.forEach(item => {
      ...
      if (matchedKey) {
        groupsMap.get(matchedKey)!.assignments.push(item);
      } else {
        ...
        if (customSubName) {
          ...
          if (!groupsMap.has(customKey)) {
            groupsMap.set(customKey, { id: customKey, name: customSubName, assignments: [] });
          }
          groupsMap.get(customKey)!.assignments.push(item);
        } else {
    ...
    const result = Array.from(groupsMap.values()).filter(grp => grp.assignments.length > 0);
    ...
    return result;
  }, [sortedAssignments, currentStage, getSubjects, selectedCollege, selectedBranch, selectedSemester]);

  const allSelected = sortedAssignments.length > 0 && sortedAssignments.every(item => selectedIds.includes(item.id));

  const handleSelectAll = () => {
    if (allSelected) {
      const sortedIds = sortedAssignments.map(n => n.id);
      setSelectedIds(prev => prev.filter(id => !sortedIds.includes(id)));
    } else {
      const newIds = new Set([...selectedIds, ...sortedAssignments.map(n => n.id)]);
      setSelectedIds(Array.from(newIds));
    }
  };

  const getNoteCountForCollege = (colId: string) => {
    if (colId === 'all') return assignments.length;
    return assignments.filter(n => (n.college || 'kiit').toLowerCase() === colId.toLowerCase()).length;
  };

  const getNoteCountForBranch = (branchId: string, colId?: string) => {
    const targetCol = colId || selectedCollege;
    if (branchId === 'all') {
      if (!targetCol || targetCol === 'all') return assignments.length;
      return assignments.filter(n => (n.college || 'kiit').toLowerCase() === targetCol.toLowerCase()).length;
    }
    return assignments.filter(n => {
  ...
  const getNoteCountForSemester = (semId: string) => {
    if (semId === 'all') return filteredAssignments.length;
    return assignments.filter(n => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative">
          ...
          <h2 className="text-2xl font-bold tracking-tight font-heading pl-3">Assignments Directory</h2>
          <p className="text-sm text-muted-foreground pl-3">
            Audit, manage, and inspect course assignment problems and homework submissions.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Choose an institution from the dynamic App Catalog to view available assignments.
              </p>
            </div>
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForCollege('all')} total assignments across all institutions
                    </p>
                  </div>
                </CardContent>
              </Card>
                ...
                <CardContent className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        View assignments across all departments in all colleges
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <Badge variant="secondary" className="text-xs font-bold px-3 py-1 bg-background">
                      {getNoteCountForBranch('all', 'all')} Assignments
                    </Badge>
                  ...
                  <Card key={collegeItem.id} className="border-border bg-card/60 overflow-hidden shadow-premium">
                    <div className="bg-accent/40 px-6 py-4 border-b border-border flex items-center justify-between flex-wrap gap-2">
                      ...
                      <Badge variant="secondary" className="font-bold text-xs bg-accent text-foreground">
                        {colCount} {colCount === 1 ? 'Assignment' : 'Assignments'}
                      </Badge>
                    </div>
                          ...
                          <CardContent className="p-4 flex flex-col justify-between h-32">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-muted-foreground mt-0.5">
                                {colCount} assignments in {collegeItem.name}
                              </p>
                            </div>
                          </CardContent>
                        </Card>
              ...
              >
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForBranch('all')} assignments across all departments
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            >
              ...
              <CardContent className="p-5 flex flex-col justify-between h-36">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {getNoteCountForSemester('all')} total assignments in branch
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      {/* STAGE 4: ASSIGNMENTS DISPLAY GROUPED BY SUBJECT */}
      {currentStage === 4 && (
        <div className="space-y-6">
          <Card className="border-border bg-card/50 backdrop-blur-sm shadow-premium">
            <CardContent className="p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  type="text"
                  placeholder="Search assignments by title, subject, uploader name, or document ID..."
                  value={searchQuery}
          ...
          {isSelectionMode && (
            <div className="flex items-center gap-2 px-4 py-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl text-xs font-semibold animate-fade-in">
              ...
              <span>Selection Mode Active — Select assignments to delete.</span>
            </div>
          )}
            ...
            <Card className="border-border overflow-hidden shadow-premium">
              <CardContent className="p-16 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mt-1">
                  {searchQuery
                    ? 'No uploaded assignments match your search parameters in this selection.'
                    : 'No assignment documents are currently available for this selection.'}
                </p>
                {searchQuery ? (
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <DialogHeader className="border-b border-border/80 pb-4 mb-4 text-left shrink-0 pr-8">
              <div className="flex items-start gap-4">
                <div className="p-4 rounded-2xl border shrink-0 bg-emerald-500/10 text-emerald-500 border-emerald-500/20">
                  <GraduationCap className="h-8 w-8" />
                </div>
                <div className="min-w-0 flex-1">
                  <DialogTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2 flex-wrap" title={selectedAssignment.title}>
                    {selectedAssignment.title || <span className="text-muted-foreground/60 italic font-normal">Untitled Document</span>}
                  ...
                  >
                    ...
                    {selectedAssignment.isVerified ? 'Verified Document' : 'Pending Verification'}
                  </div>
                </div>
              </div>
            </DialogHeader>
                ...
                {selectedAssignment.thumbnailUrl ? (
                  <div
                    className={`w-full max-w-xs h-32 bg-accent/20 rounded-xl overflow-hidden border border-border flex items-center justify-center shadow-sm relative group ${
                      selectedAssignment.fileUrl || selectedAssignment.downloadUrl ? 'cursor-pointer transition-all duration-200 hover:scale-[1.01] hover:opacity-90' : ''
                    }`}
                    onClick={() => {
                      const url = selectedAssignment.fileUrl || selectedAssignment.downloadUrl;
                      if (url) window.open(url, '_blank', 'noopener,noreferrer');
                    }}
                  >
                    <img src={selectedAssignment.thumbnailUrl} alt="Preview thumbnail" className="w-full h-full object-cover group-hover:brightness-110 transition-all duration-200" />
                    {(selectedAssignment.fileUrl || selectedAssignment.downloadUrl) && (
                ...
                <Card className="border-border bg-accent/15 p-4 flex flex-col justify-between space-y-3">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-foreground font-mono text-[10px]">{selectedAssignment.mimeType || 'application/pdf'}</span>
                    </div>
                  </div>
                </Card>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-card border border-border/80 rounded-xl p-3 text-center shadow-sm">
                    ...
                    <span className="text-lg font-bold text-foreground mt-0.5 block">
                      {(selectedAssignment.upvotes !== undefined ? selectedAssignment.upvotes : selectedAssignment.likesCount || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="text-xs font-bold flex items-center gap-1.5 h-10 bg-card border-border/80 text-foreground hover:bg-accent/20"
                    onClick={() => handleCopyText(selectedAssignment.id, 'docId')}
                  >
      ...
      <BulkDeleteDialog
        ...
        showToast={showToast}
        resources={assignments.filter(n => selectedIds.includes(n.id))}
        resourceType="assignments"
      />
