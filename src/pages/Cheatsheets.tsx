...
interface CheatsheetsSubjectGroupCardProps {
  group: { id: string; name: string; cheatsheets: CheatsheetItem[] };
  isSelectionMode: boolean;
  selectedIds: string[];
  setSelectedIds: React.Dispatch<React.SetStateAction<string[]>>;
  allSelected: boolean;
  handleSelectAll: () => void;
  handleOpenDetails: (item: CheatsheetItem) => void;
  formatFileSize: (bytes: any) => string;
}

const CheatsheetsSubjectGroupCard: React.FC<CheatsheetsSubjectGroupCardProps> = ({
  group,
  isSelectionMode,
  selectedIds,
  setSelectedIds,
  ...
  const { visibleItems, isExpanded, toggleExpand, hasMore, remainingCount } = useExpandableList(group.cheatsheets, 5);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Subject Group • {group.cheatsheets.length} {group.cheatsheets.length === 1 ? 'Cheatsheet' : 'Cheatsheets'}
            </p>
          </div>
        </div>
        <Badge variant="secondary" className="font-semibold text-xs bg-accent text-foreground">
          {group.cheatsheets.length} Cheatsheets
        </Badge>
      </div>
...
export const Cheatsheets: React.FC = () => {
  ...
  const fetchCheatsheets = async () => {
    ...
    try {
      ...
      querySnapshot.forEach(docSnap => {
        const data = docSnap.data() as CheatsheetItem;
        if (!data.temp && data.title) {
          fetched.push({
            ...data,
  ...
  const handleOpenDetails = (cheatsheet: CheatsheetItem) => {
  ...
  const filteredCheatsheets = useMemo(() => {
    return cheatsheets.filter(item => {
      ...
      if (q) {
        ...
        if (!titleMatch && !subjectMatch && !uploaderMatch && !idMatch) return false;
      }
      return true;
    });
  }, [cheatsheets, selectedCollege, selectedBranch, selectedSemester, searchQuery]);
  ...
  const sortedCheatsheets = useMemo(() => {
    return [...filteredCheatsheets].sort((a, b) => {
      if (sortBy === 'Latest') {
  ...
  }, [filteredCheatsheets, sortBy]);

  const subjectGroups = useMemo(() => {
    if (currentStage !== 4) return [];

    const catalogSubjects = getSubjects(selectedCollege, selectedBranch, selectedSemester);
    const groupsMap = new Map<string, { id: string; name: string; cheatsheets: CheatsheetItem[] }>();

    catalogSubjects.forEach(s => {
      groupsMap.set(s.id.toLowerCase(), { id: s.id, name: s.name, cheatsheets: [] });
    });

    const uncategorized: CheatsheetItem[] = [];
    ...
    sortedCheatsheets.forEach(item => {
      ...
      if (matchedKey) {
        groupsMap.get(matchedKey)!.cheatsheets.push(item);
      } else {
        ...
        if (customSubName) {
          ...
          if (!groupsMap.has(customKey)) {
            groupsMap.set(customKey, { id: customKey, name: customSubName, cheatsheets: [] });
          }
          groupsMap.get(customKey)!.cheatsheets.push(item);
        } else {
    ...
    const result = Array.from(groupsMap.values()).filter(grp => grp.cheatsheets.length > 0);

    if (uncategorized.length > 0) {
      result.push({
        id: 'uncategorized',
        name: 'Other / General Subjects',
        cheatsheets: uncategorized
      });
    }

    return result;
  }, [sortedCheatsheets, currentStage, getSubjects, selectedCollege, selectedBranch, selectedSemester]);

  const allSelected = sortedCheatsheets.length > 0 && sortedCheatsheets.every(item => selectedIds.includes(item.id));

  const handleSelectAll = () => {
    if (allSelected) {
      const sortedIds = sortedCheatsheets.map(n => n.id);
      setSelectedIds(prev => prev.filter(id => !sortedIds.includes(id)));
    } else {
      const newIds = new Set([...selectedIds, ...sortedCheatsheets.map(n => n.id)]);
      setSelectedIds(Array.from(newIds));
    }
  };

  const getNoteCountForCollege = (colId: string) => {
    if (colId === 'all') return cheatsheets.length;
    return cheatsheets.filter(n => (n.college || 'kiit').toLowerCase() === colId.toLowerCase()).length;
  };

  const getNoteCountForBranch = (branchId: string, colId?: string) => {
    const targetCol = colId || selectedCollege;
    if (branchId === 'all') {
      if (!targetCol || targetCol === 'all') return cheatsheets.length;
      return cheatsheets.filter(n => (n.college || 'kiit').toLowerCase() === targetCol.toLowerCase()).length;
    }
    return cheatsheets.filter(n => {
  ...
  const getNoteCountForSemester = (semId: string) => {
    if (semId === 'all') return filteredCheatsheets.length;
    return cheatsheets.filter(n => {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="relative">
          ...
          <h2 className="text-2xl font-bold tracking-tight font-heading pl-3">Cheatsheets Directory</h2>
          <p className="text-sm text-muted-foreground pl-3">
            Audit, verify, and manage quick-reference formulas, summary sheets, and exam code sheets.
          </p>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Choose an institution from the dynamic App Catalog to view available cheatsheets.
              </p>
            </div>
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForCollege('all')} total cheatsheets across all institutions
                    </p>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <CardContent className="p-5 flex flex-col justify-between h-36">
                      <div className="flex items-start justify-between">
                        ...
                        <Badge variant="secondary" className="text-[10px] font-bold">
                          {count} {count === 1 ? 'Cheatsheet' : 'Cheatsheets'}
                        </Badge>
                      </div>
                      <div>
                        <h4 className="font-bold text-foreground group-hover:text-violet-400 transition-colors text-base flex items-center justify-between">
                ...
                <CardContent className="p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        View cheatsheets across all departments in all colleges
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 shrink-0">
                    <Badge variant="secondary" className="text-xs font-bold px-3 py-1 bg-background">
                      {getNoteCountForBranch('all', 'all')} Cheatsheets
                    </Badge>
                  ...
                  <Card key={collegeItem.id} className="border-border bg-card/60 overflow-hidden shadow-premium">
                    <div className="bg-accent/40 px-6 py-4 border-b border-border flex items-center justify-between flex-wrap gap-2">
                      ...
                      <Badge variant="secondary" className="font-bold text-xs bg-accent text-foreground">
                        {colCount} {colCount === 1 ? 'Cheatsheet' : 'Cheatsheets'}
                      </Badge>
                    </div>
                          ...
                          <CardContent className="p-4 flex flex-col justify-between h-32">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-muted-foreground mt-0.5">
                                {colCount} cheatsheets in {collegeItem.name}
                              </p>
                            </div>
                          </CardContent>
                        </Card>
              ...
              >
                ...
                <CardContent className="p-5 flex flex-col justify-between h-36">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      {getNoteCountForBranch('all')} cheatsheets across all departments
                    </p>
                  </div>
                </CardContent>
              </Card>
            ...
            >
              ...
              <CardContent className="p-5 flex flex-col justify-between h-36">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {getNoteCountForSemester('all')} total cheatsheets in branch
                  </p>
                </div>
              </CardContent>
            </Card>
      ...
      {/* STAGE 4: CHEATSHEETS DISPLAY GROUPED BY SUBJECT */}
      {currentStage === 4 && (
        <div className="space-y-6">
          <Card className="border-border bg-card/50 backdrop-blur-sm shadow-premium">
            <CardContent className="p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
              <div className="relative flex-1">
                ...
                <Input
                  type="text"
                  placeholder="Search cheatsheets by title, subject, uploader name, or document ID..."
                  value={searchQuery}
          ...
          {isSelectionMode && (
            <div className="flex items-center gap-2 px-4 py-3 bg-amber-500/10 border border-amber-500/20 text-amber-500 rounded-xl text-xs font-semibold animate-fade-in">
              ...
              <span>Selection Mode Active — Select cheatsheets to delete.</span>
            </div>
          )}
            ...
            <Card className="border-border overflow-hidden shadow-premium">
              <CardContent className="p-16 text-center flex flex-col items-center justify-center">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mt-1">
                  {searchQuery
                    ? 'No uploaded cheatsheets match your search parameters in this selection.'
                    : 'No cheatsheet documents are currently available for this selection.'}
                </p>
                {searchQuery ? (
          ...
          <div className="flex flex-col flex-1 min-h-0">
            <DialogHeader className="border-b border-border/80 pb-4 mb-4 text-left shrink-0 pr-8">
              <div className="flex items-start gap-4">
                <div className="p-4 rounded-2xl border shrink-0 bg-blue-500/10 text-blue-500 border-blue-500/20">
                  <FileCode className="h-8 w-8" />
                </div>
                <div className="min-w-0 flex-1">
                  <DialogTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2 flex-wrap" title={selectedCheatsheet.title}>
                    {selectedCheatsheet.title || <span className="text-muted-foreground/60 italic font-normal">Untitled Document</span>}
                  ...
                  >
                    ...
                    {selectedCheatsheet.isVerified ? 'Verified Document' : 'Pending Verification'}
                  </div>
                </div>
              </div>
            </DialogHeader>
                ...
                {selectedCheatsheet.thumbnailUrl ? (
                  <div
                    className={`w-full max-w-xs h-32 bg-accent/20 rounded-xl overflow-hidden border border-border flex items-center justify-center shadow-sm relative group ${
                      selectedCheatsheet.fileUrl || selectedCheatsheet.downloadUrl ? 'cursor-pointer transition-all duration-200 hover:scale-[1.01] hover:opacity-90' : ''
                    }`}
                    onClick={() => {
                      const url = selectedCheatsheet.fileUrl || selectedCheatsheet.downloadUrl;
                      if (url) window.open(url, '_blank', 'noopener,noreferrer');
                    }}
                  >
                    <img src={selectedCheatsheet.thumbnailUrl} alt="Preview thumbnail" className="w-full h-full object-cover group-hover:brightness-110 transition-all duration-200" />
                    {(selectedCheatsheet.fileUrl || selectedCheatsheet.downloadUrl) && (
                ...
                <Card className="border-border bg-accent/15 p-4 flex flex-col justify-between space-y-3">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div>
                      ...
                      <span className="font-semibold text-foreground font-mono text-[10px]">{selectedCheatsheet.mimeType || 'application/pdf'}</span>
                    </div>
                  </div>
                </Card>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="bg-card border border-border/80 rounded-xl p-3 text-center shadow-sm">
                    ...
                    <span className="text-lg font-bold text-foreground mt-0.5 block">
                      {(selectedCheatsheet.upvotes !== undefined ? selectedCheatsheet.upvotes : selectedCheatsheet.likesCount || 0).toLocaleString()}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  ...
                  <Button
                    ...
                    size="sm"
                    className="text-xs font-bold flex items-center gap-1.5 h-10 bg-card border-border/80 text-foreground hover:bg-accent/20"
                    onClick={() => handleCopyText(selectedCheatsheet.id, 'docId')}
                  >
      ...
      <BulkDeleteDialog
        ...
        showToast={showToast}
        resources={cheatsheets.filter(n => selectedIds.includes(n.id))}
        resourceType="cheatsheets"
      />
